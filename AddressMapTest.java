...
public class AddressMapTest {
	...
	public AddressMapTest(String add1, String add2, String city, String state, Integer pinCode) {
		// TODO Auto-generated constructor stub
	}

	public static void add(Map<String, Map<String, List<AddressMapTest>>> stateMap, AddressMapTest ad) {
		String state =ad.getState();
		String city = ad.getCity();
		List<AddressMapTest> li = new ArrayList<>();
		li.add(ad);

		Map<String, List<AddressMapTest>> cityMap = new HashMap<String, List<AddressMapTest>>();
		cityMap.put(city, li);
	...
	private static void search(Map<String, Map<String, List<AddressMapTest>>> stateMap, String state, String city) {
		if (stateMap.containsKey(state)) {
			...
			if (stateMap.get(state).containsKey(city)) {
				for (AddressMapTest m : stateMap.get(state).get(city)) {
	...
	public static void main(String[] args) {
		...
		Map<String, Map<String, List<AddressMapTest>>> map = new HashMap<>();
		...
		for (int i = 1; i <= noOfAddress; i++) {
			...
			// List<AddressMap> lid = new ArrayList<AddressMap>();
			// li.add();

			// Map<String, List<AddressMap>> cityMap = new
			// HashMap<String,List<AddressMap>>();

			// cityMap.put(city, lid);
			add(map, new AddressMapTest(add1, add2, city, state, pinCode));
		}
